...
contract CollateralPool {
    ...
    /// @param _amount The amount of collateral to deposit (18 decimals).
    function depositCollateral(uint256 _amount) external {
        require(_amount >= 1**(18-_COLLATERALTOKEN_DECIMALS), "Does not respect collateral Token decimals");
        IERC20x(_COLLATERALTOKEN).transferFrom(msg.sender, address(this), toCollateralTokenDecimals(_amount));
        _userInfos[msg.sender].collateral += _amount;
    ...
    /// @param _amount The amount of collateral to withdraw (18 decimals).
    function withdrawCollateral(uint256 _amount) external {
        ...
        IERC20x(_COLLATERALTOKEN).transfer(msg.sender, toCollateralTokenDecimals(_amount));
        _userInfos[msg.sender].collateral -= _amount;
    ...
    function claimRewards(uint256 _index, uint256 _id) external {

        // Allowed to claim ?
        address idOwner = IERC721x(IMarketPool(IMain(_MAIN).getIdToMarket(_index)).getERC721_LP()).ownerOf(_id);
        require(tx.origin == idOwner, "You are not the owner of this LP");
        ...
        emit RewardsClaimed(idOwner, _index, _id, rewards);
    }
