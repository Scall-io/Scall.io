...
contract MarketPool {
    ...
    constructor(address _main, address _tokenA, uint8 _tokenADecimals, address _tokenB, uint8 _tokenBDecimals, address _pricefeed, uint256 _pricefeedDecimal, uint256 _range, uint256 _yield) {
        _MAIN = _main;
        _TOKENA = _tokenA;
    ...
    /// @param _amount The amount of assets to deposit (18 decimals)
    function deposit(bool _isCall, uint256 _amount) external {
        require(_amount >= 1e4, "_amount too low");
        ...
        if (_isCall) {
            require(_amount >= 1**(18-_TOKENA_DECIMALS), "Does not respect tokenA decimals");
            IERC20x(_TOKENA).transferFrom(msg.sender, address(this), toTokenADecimals(_amount));
            strike = interval[1];
            _strikeToInfos[strike].callLP += _amount;
        } else {
            require(_amount >= 1**(18-_TOKENB_DECIMALS), "Does not respect tokenB decimals");
            IERC20x(_TOKENB).transferFrom(msg.sender, address(this), toTokenBDecimals(_amount));
            strike = interval[0];
    ...
    /// @param _isCall Specifies if the option is a call or a put
    /// @param _amount The amount of assets for the option contract (18 decimals)
