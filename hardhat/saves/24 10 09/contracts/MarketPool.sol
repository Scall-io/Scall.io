...
contract MarketPool {
    ...
    function getPrice() public view returns(uint256) {
        ...
        uint256 price = uint256(result) * (1e18/10**_PRICEFEED_DECIMAL);
        return price;
    }
    ...
    function getStrikeInfos() public view returns(StrikeInfos memory) {
        ...
        //For all open LP
        for (uint256 i ; i < IERC721x(_ERC721_LP).totalSupply() ; i++) {
        ...
        //For all open trade
        for (uint256 ii ; ii < IERC721x(_ERC721_CONTRACT).totalSupply() ; ii++) {
        ...
        StrikeInfos memory strikeInfos = StrikeInfos(callLP, callLU, putLP, putLU);
