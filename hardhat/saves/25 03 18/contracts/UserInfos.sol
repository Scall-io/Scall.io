...
contract UserInfos {
    ...
    function GetUserLpsForMarket(uint256 _index, address _user) public view returns(UserLp[] memory) {
        IMarketPool marketPool = IMarketPool(IMain(_MAIN).getIdToMarket(_index));
        ...
        UserLp[] memory userLpList = new UserLp[](balanceOfUser);

        for (uint256 i = 0; i < balanceOfUser; i++) {
            uint256 ID = ERC721_LP.tokenOfOwnerByIndex(_user, i);
            IMarketPool.LpInfos memory userLpInfos = marketPool.getLpInfos(ID);

            (uint256 withdrawableTokenA, uint256 withdrawableTokenB) = GetWithdrawableForLPForMaket(_index, ID);
