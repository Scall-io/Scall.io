...
contract CollateralPool {
    ...
    /// @param _user The address of the user.
    /// @return The total fees accumulated for the user.
    ...
    /// @return The net rewards after fees.
    ...
    function canOpenContract(address _user, uint256 _rent) external view returns(bool) {
        uint256 liqThresh = IMain(_MAIN).getMinCollateral();
    ...
    function claimRewards(uint256 _index, uint256 _id) external {
        ...
        // Transfer
        IERC20x(_COLLATERALTOKEN).transfer(idOwner, rewards - feeAmount);
        IERC20x(_COLLATERALTOKEN).transfer(IOwnable(_MAIN).owner(),feeAmount);
    ...
    /// @param _index The market index.
    /// @param _id The ID of the user's position.
    /// @return The penalty amount awarded to the liquidator.
